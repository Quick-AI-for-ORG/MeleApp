...
const CRUDInterface = {
  create: async function (data, model, compareKey) {
    try {
      ...
      if (!existingRecord) {
        ...
        await Log.create({
          log: `Created new record with ${compareKey}: ${data[compareKey]}`,
          degree: 1,
        });
        return new Result(
          1,
          record._doc,
          `Created new record with ${compareKey}: ${data[compareKey]}`
        );
      } else {
        await Log.create({
          log: `Record with ${compareKey}: ${data[compareKey]} already exists. Skipping creation.`,
          degree: 0,
        });
        return new Result(
          0,
          null,
          `Record with ${compareKey}: ${data[compareKey]} already exists. Skipping creation.`
        );
      }
    } catch (error) {
  ...
  modify: async function (primaryKey, newData, model, compareKey) {
    try {
      ...
      let record = await models[model].findOneAndUpdate(
        compareClause,
        { $set: newData },
        { new: true }
      );
      if (!record) {
        await Log.create({
          log: `Record with ${compareKey}: ${primaryKey} not found. Skipping modification.`,
          degree: 0,
        });
        return new Result(
          0,
          null,
          `Record with ${compareKey}: ${primaryKey} not found. Skipping modification.`
        );
      } else {
        await Log.create({
          log: `Updated record with ${compareKey}: ${newData[compareKey]}`,
          degree: 1,
        });
        return new Result(
          1,
          record._doc,
          `Updated record with ${compareKey}: ${newData[compareKey]}`
        );
      }
    } catch (error) {
  ...
  remove: async function (primaryKey, model, compareKey) {
    try {
      ...
      if (!record) {
        await Log.create({
          log: `Record with ${compareKey}: ${primaryKey} not found. Skipping deletion.`,
          degree: 0,
        });
        return new Result(
          0,
          null,
          `Record with ${compareKey}: ${primaryKey} not found. Skipping deletion.`
        );
      } else {
        await Log.create({
          log: `Deleted record with ${compareKey}: ${primaryKey}`,
          degree: 1,
        });
        return new Result(
          1,
          record._doc,
          `Deleted record with ${compareKey}: ${primaryKey}`
        );
      }
    } catch (error) {
  ...
  removeAll: async function (primaryKey, model, compareKey) {
    try {
      ...
      if (result.deletedCount === 0) {
        await Log.create({
          log: `No records found with ${compareKey}: ${primaryKey}. Skipping deletion.`,
          degree: 0,
        });
        return new Result(
          0,
          null,
          `No records found with ${compareKey}: ${primaryKey}. Skipping deletion.`
        );
      } else {
        await Log.create({
          log: `Deleted ${result.deletedCount} records with ${compareKey}: ${primaryKey}`,
          degree: 1,
        });
        return new Result(
          1,
          result,
          `Deleted ${result.deletedCount} records with ${compareKey}: ${primaryKey}`
        );
      }
    } catch (error) {
      await Log.create({ log: `Error removing data: ${error}`, degree: -1 });
  ...
  get: async function (primaryKey, model, compareKey) {
    try {
      ...
      if (record) {
        await Log.create({
          log: `Found record with ${compareKey}: ${primaryKey}`,
          degree: 1,
        });
        return new Result(
          1,
          record._doc,
          `Found record with ${compareKey}: ${primaryKey}`
        );
      } else {
        await Log.create({
          log: `Record with ${compareKey}: ${primaryKey} not found.`,
          degree: 0,
        });
        return new Result(
          0,
          null,
          `Record with ${compareKey}: ${primaryKey} not found.`
        );
      }
    } catch (error) {
  ...
  getAll: async function (model) {
    try {
      ...
      await Log.create({
        log: `Found ${records.length} records in ${model}.`,
        degree: 1,
      });
      return new Result(
        1,
        resultData,
        `Found ${records.length} records in ${model}..`
      );
    } catch (error) {
  ...
  getAllFiltered: async function (primaryKey, model, compareKey) {
    try {
      ...
      const resultData = records.map((record) => record._doc) || [];
      await Log.create({
        log: `Found ${records.length} records in ${model} with ${compareClause}.`,
        degree: 1,
      });
      return new Result(
        1,
