...
class tempForecast:
    ...
    def loadAndPreprocessData(self,inputDates,inputTemps,freq):
        data = pd.DataFrame({"Date_Time": inputDates,"Temperature":inputTemps})
        data['Date_Time'] = pd.to_datetime(data['Date_Time'])
        
        data = data.drop_duplicates(subset='Date_Time').sort_values(by='Date_Time').reset_index(drop=True)
        
        data = data.set_index('Date_Time').asfreq(freq)
        
        data = data.interpolate(method='time')  
        return data
    
    def splitData(self,data, targetColumn, splitRatio=0.8):
    ...
    def trainArimaModel(self,trainData, order=(1, 1, 1)):
        model = ARIMA(trainData, order=order)
        modelFit = model.fit()
        return modelFit
 
    def generateForecast(self,model, steps):
    ...
    def printMetrics(self,actual, forecast, modelName="ARIMA"):
        mse = mean_squared_error(actual, forecast)
        mad = mean_absolute_error(actual, forecast)
        smapeVal = self.smape(actual, forecast)
    ...
    def plotForecastsVsActual(self,dates, actual, forecasted, modelName="ARIMA", temp_min=-19.97, temp_max=40.00):
